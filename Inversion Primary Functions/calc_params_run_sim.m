function out = calc_params_run_sim(data, wr_load, P, tune, useTrueSpeed)
...
% Define total stator inductance 'Las', total rotor inductance 'Lar', and convenience parameter 'D'
Las = Lm + Ls;
Lar = Lm + Lr;
D = Lm*Lm - Las*Lar;
...
% Notice 'Lal' used now for total inductances.
Las = Lm + Lal;
Lar = Lm + Lal;
D = Lm^2 - Las*Lar;
...
% Nested function that computes the objective function
    ...
    function [error, J, Wr_est, sim_state] = error_function_mean_speed_with_Jacobian_Rot_and_Load(Parameters)
        ...
        % make sure Load variables have the longer vector
        ...
        % Combine numberator/denominator:
        ...
        % Refer to slide 29 of Chris's defense talk. On the slide, the hat symbol (^) = estimate and Beta = tune. 
        % We are NOT using true speed (first condition) for the induction motor.
        if ~useTrueSpeed
